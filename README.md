...
- `tests/strava-verify.test.ts` — holds every `EVENTS` row that names a Strava activity
  ...
  as a broken site. It catches the two things a hand-typed figure gets wrong — a screenshot
  taken before the activity was edited, and a distance converted by rounding where Strava's
  page truncates. The token needs `activity:read_all`; a detailed read answers 404, not 403,
  when the scope is missing, so an under-scoped token looks exactly like a wrong id.
