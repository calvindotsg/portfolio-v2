...
- `tests/strava-verify.test.ts` — holds every `EVENTS` row that names a Strava activity
  ...
  as a broken site. It catches the two things a hand-typed figure gets wrong — a time that
  matches no recording on the day it claims, and a distance converted the other way from the
  rule, since `km` is metres rounded half-up to two places and truncating instead puts four of
  these rows out by 0.01. The token needs `activity:read_all`; a detailed read answers 404, not
  403, when the scope is missing, so an under-scoped token looks exactly like a wrong id.
