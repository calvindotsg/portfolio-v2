...
import {
    EVENTS, GOAL_YEAR, GOALS, NEW_TAB_NOTICE, PATCHES, raceKm, type RaceEvent, type Recording,
    recordingKm, recordingsOf, type Sport, stravaActivityUrl,
...
describe("dist/patches", () => {
    ...
    it("prints every distance to two decimals, split so the fraction can be set small", () => {
        for (const {bib, event} of wallBibs(PAGES.all)) {
            ...
            expect(value.textContent?.replace(/\s+/g, ""), `${event.name} distance`).toBe(raceKm(event).toFixed(2));
            expect(value.querySelector(".bib-fraction")?.textContent, `${event.name} fraction`)
                .toBe(`.${raceKm(event).toFixed(2).split(".")[1]}`);
        }
    });
