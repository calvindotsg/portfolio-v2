 ...
 *
 * THIS ONE THEMES ITSELF, WHICH IS WHY IT IS THE ONE THE TAB LOADS. An SVG used as an icon
 * still evaluates its own `<style>`, so the light pair is the default and a
 * `prefers-color-scheme: dark` block swaps to the dark pair. `mark-light.svg` and
 * `mark-dark.svg` exist beside it for a consumer that cannot evaluate CSS at all — a raster
 * pipeline, a README, a card generator — and neither is what a browser is pointed at.
 *
